...
async function getInitialPrice() {
  ...
  const wsb = "wsb"
  const sfin_ADDRESS = "0x0d94e59332732d18cf3a3d457a8886a2ae29ea1b"
  const sfin = "sfin"
  ...
  let sfinSGBPrice = (Number(arryPrice[1]._hex) / 10 ** 18).toFixed(5)
  let wsbUSDPrice = (sgbPrice / wsbSGBPrice).toFixed(5)
  let sfinUSDPrice = (sgbPrice / sfinSGBPrice).toFixed(5)
  console.log(wsb + " USD price is " + wsbUSDPrice)
  console.log(sfin + " USD price is " + sfinUSDPrice)

  clearRoles()
  lastPrice = sfinUSDPrice || 0
  let symbol = `${process.env.COIN_ID.toUpperCase()}`
  client.user.setPresence({
    activities: [{
    name: `SGB=${sfinSGBPrice} ${symbol}`,
    type: `PLAYING`
    }]
  })

  arrow = mid
  client.guilds.cache.find(guild => guild.id === process.env.SERVER_ID).me.setNickname(`${symbol} ${arrow} ${process.env.CURRENCY_SYMBOL}${sfinUSDPrice}`)
